  ...
  async show(users, user) {
    ...
    return new Promise((resolve) => {
      ...
      form.addEventListener('submit', async (e) => {
        ...
        if (inputActive == null) {
          error.textContent = _t(
            'Please enter a username or select an existing user.'
          )
        } else if (inputActive) {
          const username = input.value.trim()
          if (!username) {
            error.textContent = _t('Please enter a username.')
            return
          }
          ...
          try {
            ...
            if (resp.status >= 300) {
              let message = _t('Error creating user: {status} {statusText}', {
                status: resp.status,
          ...
          error.textContent = _t('Please select an existing user.')
          return
        } else {
          resolve({
